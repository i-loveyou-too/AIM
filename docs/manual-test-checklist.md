...
## 사전 준비
- [x] 프론트 실행 확인 (`/login` 접속 가능)
- [x] 백엔드 실행 확인 (`/api/auth/csrf` 200)
- [x] 테스트 DB 연결 확인 (`DB_NAME=aimon_test_mvp`)
- [x] teacher 계정 로그인 가능 확인
...
## 점검 메모 (2026-04-02)
- 로컬 사용자 환경에서는 `/api/auth/csrf`, `login`, `me` 성공 응답 확인됨.
- 에이전트 실행 환경에서는 외부 DB(`192.168.99.138:5432`) 타임아웃이 간헐 발생해, 교사 API 전체 200 재검증은 사용자 터미널 기준으로 재확인 필요.
