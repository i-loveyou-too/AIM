      ...
      <div className="mt-4 grid gap-2.5 sm:grid-cols-2">
        {items.map((item) => (
          <button
            key={item.label}
            type="button"
            className="group rounded-[24px] border border-border bg-[#f8f8fa] p-3.5 text-left transition duration-200 hover:-translate-y-0.5 hover:border-brand/30 hover:bg-white hover:shadow-glow"
          >
            <div className="flex items-start justify-between gap-4">
              <span
                className={`flex h-10 w-10 items-center justify-center rounded-xl text-base font-semibold ${toneStyles[item.tone]}`}
              >
                {item.icon}
              </span>
              <span className="text-base text-muted transition group-hover:translate-x-0.5 group-hover:text-brand">
                →
              </span>
            </div>

            <p className="mt-3 text-base font-extrabold tracking-tight text-text">
              {item.label}
            </p>
