  ...
  return (
    <section className="rounded-[32px] border border-border/80 bg-white p-5 shadow-soft">
      <div>
        ...
        <h2 className="mt-2 text-xl font-semibold text-text">방금 반영된 변화를 빠르게 확인합니다</h2>
      </div>

      <div className="mt-5 space-y-4">
        {items.map((item, index) => (
          <div key={`${item.time}-${item.student}-${index}`} className="flex gap-4">
            ...
            <article className="flex-1 rounded-2xl border border-border bg-[#fafafa] p-4 transition duration-200 hover:border-brand/30 hover:shadow-glow">
              ...
              <p className="mt-3 text-sm font-medium text-brand">{item.student}</p>
            </article>
          </div>
        ))}
      </div>
